...
public sealed partial class SenderForm : Form
{
    ...
    public SenderForm()
    {
        Text = "LAN Sender - Step 9 Realtime Screen Monitor";
        Width = 1160;
        Height = 820;
    ...
    private void BuildUi()
    {
        ...
        _screenIntervalBox.Minimum = 16;
        _screenIntervalBox.Maximum = 1000;
        _screenIntervalBox.Increment = 1;
        _screenIntervalBox.Value = 33;
        ...
        _screenStatusLabel.Text = "Realtime screen idle";
    ...
    private async Task StartScreenStreamAsync(bool selectedOnly)
    {
        ...
        SetSendButtonsEnabled(_serverCts is not null);
        UpdateScreenStatus("Realtime monitor starting");
    ...
    private async Task ScreenStreamLoopAsync(bool useFixedTargets, List<ClientConnection> fixedTargets, CancellationToken token)
    {
        ...
        try
        {
            while (!token.IsCancellationRequested)
            {
                ...
                if (targets.Count == 0)
                {
                    UpdateScreenStatus("Realtime monitor stopped: no clients");
                    break;
                }
                ...
                ulong currentHash = ComputeFnv1a64(capture.ImageBytes);
                bool changed = !hasLastHash || currentHash != lastHash;
                bool heartbeat = (DateTime.UtcNow - lastHeartbeatAt).TotalMilliseconds >= 1000;
                ...
                if (changed || heartbeat)
                {
                    ...
                    foreach (ClientConnection connection in targets)
                    {
                        ...
                        {
                            success++;
                            totalSentBytes += capture.ImageBytes.Length;
                        }
                    }
                    ...
                    double seconds = Math.Max(stopwatch.Elapsed.TotalSeconds, 0.001);
                    double fps = sentFrames / seconds;
                    double mbps = (totalSentBytes * 8.0) / seconds / 1_000_000.0;

                    UpdateScreenStatus(
                        $"Realtime: frame {capture.FrameNo}, captured {capturedFrames}, sent {sentFrames}, skipped {skippedFrames}, {capture.Width}x{capture.Height}, {FormatBytes(capture.ImageBytes.Length)}, clients {success}/{targets.Count}, {fps:0.0} fps, {mbps:0.0} Mbps");
                }
                else
                {
                    skippedFrames++;
                    ...
                    UpdateScreenStatus(
                        $"Realtime: no change, captured {capturedFrames}, sent {sentFrames}, skipped {skippedFrames}, {fps:0.0} fps, {mbps:0.0} Mbps");
                }
        ...
        {
            UpdateScreenStatus("Realtime monitor stopped");
        }
        catch (Exception ex)
        {
            AddLog($"Realtime monitor error: {ex.Message}");
            UpdateScreenStatus("Realtime monitor error");
        }
        finally
        {
            stopwatch.Stop();
            ...
            if (!IsDisposed)
            {
                if (InvokeRequired)
                {
                    BeginInvoke(new Action(() => _stopScreenButton.Enabled = false));
                }
                else
                {
                    _stopScreenButton.Enabled = false;
    ...
    private void StopScreenStream()
    {
        ...
        AddLog("Realtime monitor stop requested.");
    }
    ...
    private static void SaveJpeg(Image image, Stream stream, int quality)
    {
        ...
        encoderParameters.Param[0] = new EncoderParameter(
            ...
            Math.Clamp((long)quality, 20L, 95L));
    ...
    private void StopServer()
    {
        ...
        finally
        {
            ...
            if (!IsDisposed)
            {
                ...
                UpdateScreenStatus("Realtime screen idle");
